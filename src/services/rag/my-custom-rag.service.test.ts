...
import type { VectorDBService, QueryMatch } from '../vector-db';
import type {
...
describe('MyCustomRAGService', () => {
	...
	describe('ask()', () => {
		...
		it('should include multiple parents for children from different parents', async () => {
			...
			expect(promptArg).toContain('Boil water in a large pot. Add salt. Add pasta.');
			expect(promptArg).toContain('Preheat oven to 350F. Prepare baking sheet.');
